...
.PHONY: serve run model test lint fmt typecheck check install install-cli up down logs \
        up-cuda down-cuda frontend-install frontend-build frontend-dev release release-dry
...
# Cut the next minor release automatically: bump version, regenerate the
# CHANGELOG from commits, run checks, then tag and push (which triggers CI to
# build the image and publish the GitHub Release). See scripts/release.py.
release:
	python3 scripts/release.py

# Preview the next version and generated changelog without releasing.
release-dry:
